...
class trainingMaskGenerator():
    ...
    def generate_mask(self, image_folder_path: str, mask_folder_path: str):
        """
        This function is used to generate masks for the purposes of training a SAM model or for pixel intensity measurement.
        @param image_folder_path: The path to the folder containing the images from which masks are to be generated.
        @param mask_folder_path: The path to the folder where the masks are to be saved.
        """
        ...
        def draw_rectangle(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                cv2.rectangle(temp_img, rect_points[0], rect_points[1], (0, 255, 0), 2)
                cv2.imshow("Image", temp_img)

                # Update input_box with the coordinates
                input_box = np.array([rect_points[0][0], rect_points[0][1], rect_points[1][0], rect_points[1][1]])
        ...
        for img in tqdm(image_files):
            ...
            with open(mask_folder / f'{img.replace('.png', '')}.pkl', 'wb') as f:
